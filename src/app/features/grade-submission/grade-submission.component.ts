...
export class GradeSubmissionComponent {
  ...
  constructor() {
    this.submitClick$.pipe(
      // exhaustMap: የመጀመሪያው ጥሪ አልቆ ምላሽ እስከሚመለስ ድረስ
      // የሚመጡ አዳዲስ የ Click ሁነቶችን (rage-clicks) በሙሉ ይሰርዛቸዋል!
      exhaustMap(payload => {
        this.isSubmitting = true;
        this.submissionStatus = 'Submitting grade to server...';
        return this.api.postGrade(payload);
      }),
      // takeUntilDestroyed: Component-ኡ ሲጠፋ Subscription-ኡን በራስ-ሰር ያቆማል
      takeUntilDestroyed()
    ).subscribe({
      next: result => {
